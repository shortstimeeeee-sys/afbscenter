# ngrok 터널 시작 스크립트 (고정 도메인 사용) - 새 창에서 실행
...
if (-not $serverRunning) {
    ...
    Write-Host "먼저 서버를 시작해주세요: .\start-server.ps1" -ForegroundColor Yellow
    Write-Host ""
...
# ngrok 실행 (고정 도메인 사용) - 새 창에서 실행
Write-Host "ngrok 실행 중 (고정 도메인: $fixedDomain)..." -ForegroundColor Green
Write-Host "새 PowerShell 창이 열립니다..." -ForegroundColor Yellow
Write-Host ""
...
Write-Host "  1. 새로 열린 ngrok 창에서 'Forwarding' 줄 확인" -ForegroundColor White
Write-Host "  2. 또는 브라우저에서 http://127.0.0.1:4040 접속" -ForegroundColor White
Write-Host ""
Write-Host "잠시 후 터널이 생성됩니다 (약 5-10초 소요)" -ForegroundColor Yellow
Write-Host ""
Write-Host "터널이 생성되면 https://$fixedDomain 으로 접속할 수 있습니다." -ForegroundColor Green
Write-Host ""
